#!/usr/bin/env python3
"""Review a 90-degree wrist turn and horizontal contact from frozen pregrasp."""
import json
import os
import signal
import subprocess
...
def main():
    ...
    try:
        ...
        subprocess.run(ros_command([
            ...
            "-p", "horizontal_continuation:=true",
            "-p", f"planning_start_trajectory_path:={run / 'pregrasp.json'}",
            "-p", f"cube_center_base_mm:={list(target['cube_center_base_mm'])}",
            "-p", f"wrist_output_path:={run / 'horizontal_wrist.json'}",
            "-p", f"contact_output_path:={run / 'horizontal_contact.json'}",
        ]), cwd=ROOT, check=True)
        print(f"HORIZONTAL PLAN-ONLY PASSED: {run}")
        input("请在RViz检查J6转90度和水平推进；按Enter关闭场景...")
    finally:
